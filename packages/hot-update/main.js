...
(function () {
    if (typeof window.jsb === 'object') {
        ...
        if (hotUpdateSearchPaths) {
            ...
            // add by anian.解决覆盖安装后，读取不到热更目录下的文件的问题
            var hotUpdateStorageFolder = localStorage.getItem('HotUpdateStorageFolder');
            if (hotUpdateStorageFolder) {
                for (let i = 0; i < paths.length; i++) {
                    if (paths[i].indexOf(hotUpdateStorageFolder) !== -1) {
                        paths.splice(i, 1)
                        i--
                    }
                }
                paths.unshift(jsb.fileUtils.getWritablePath() + hotUpdateStorageFolder + '/')
            }
